...
namespace MyGame
{
    public class MultiDrawerController : MonoBehaviour, IInteractable
    {
        [Header("Drawer Settings")]
        public Transform[] drawerTransforms;  // Assign the target drawer objects in the Inspector
        public float[] moveDistances;  // Distances to move each drawer along the Z axis
        ...
        private void Start()
        {
            ...
            if (outline == null)
            {
                ...
                if (outline == null)
                {
                    Debug.LogError("No Outline component found on the MultiDrawerController or its children.");
                }
            }
        ...
        {
            Vector3 targetPosition = isOpen[index] ? initialPositions[index] : initialPositions[index] + new Vector3(0, 0, moveDistances[index]);
